...
/**
 * Emails the Invoice with payment option from QuickBooks to the address supplied
 * or the specified 'sendTo' address
 *
 * @param  {string} id - The Id of the invoice
 * @param  {string} sendTo - email address to send the Invoice to
 * @param  {function} callback - Callback function which is called with any error and the updated Invoice
 */

// /invoice/<invoiceId>/send?sendTo=<emailAddr>
QuickBooks.prototype.emailInvoice = function (id, sendTo, callback) {
  var path = '/invoice/' + id + '/send?sendTo=' + sendTo
  module.request(this, 'post', { url: path }, null, module.unwrap(callback, 'Invoice'))
}
